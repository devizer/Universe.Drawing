namespace Universe.Bitmap
{
    ...
    {
        unsafe public static Bitmap2 Read(Stream input)
        {
            ...
            byte[] arrInfoHeaderSize = StreamExtentions.Read(input, 4, "BITMAP-INFO-HEADER's header size is expected");
            ...
            {
                // v3
                byte[] arrInfoHeader2 = StreamExtentions.Read(input, sizeof(BitmapInfoHeader) - 4, "BITMAP-INFO-HEADER, which length is 40, is expected");
                byte[] arrInfoHeader = StreamExtentions.JoinBytes(arrInfoHeaderSize, arrInfoHeader2);
                ...
                int pos = sizeof (BITMAPFILEHEADER) + sizeof (BitmapInfoHeader);
                int paletteLength = bfh.bfOffBits - pos;
                if (paletteLength > 0)
                {
                ...
                Bitmap2 ret = new Bitmap2(infoHeader.biWidth, infoHeader.biHeight, format);
...
*/                    
        ...
        static void ReadPixels(Stream stream, int pos, Bitmap2 bitmap)
        {
            var stride = bitmap.Stride;
            byte[] buffer = new byte[stride];
            int len = stride * bitmap.Height;
            IntPtr p = bitmap.Scan0;
